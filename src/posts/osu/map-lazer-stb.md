...
然而，目前的 lazer 编辑器存在一些问题：

- 没有故事板编辑器
- 谱面文件无法直接访问（在外部编辑状态下能够临时挂载）

如果你很在意上面的问题，想在 osu!stable 里测试你的图，但是依然希望保留 osu!lazer 既有的编辑体验，那么这篇文章可能适合你。<!-- more -->
...
![导出菜单](./img/export-menu.jpg)

稍后会弹出一个通知，提示导出已完成。点击这个通知，会帮你打开导出文件所在位置。如果没打开也无所谓，一般情况下，Windows 版本 osu!lazer 所有导出文件的位置在 `C:\Users\<你的用户名>\AppData\Roaming\osu\exports`（即 osu! 数据目录，点击导出通知可以查看）。

然后选中这个文件，选用 **osu!stable** 打开。在打开方式菜单中显示的都是 `osu!`，但是从图标上很容易看出来（深色的就是）。

::: important

若在 osu!lazer 编辑并保存过该谱面，启动稳定版后，会显示通知“无法解析谱面头部，谱面可能已损坏！”，进入编辑器则会报错。

![报错窗口](./img/editor-error.jpg)

此时需要手动修复谱面文件中的问题，请继续阅读。
...
在新版本 osu!lazer 导出的谱面中，问题通常出现在 `.osu` 文件的 `[Events]` 小节，你会发现多了几行缺少参数、明显很不同的命令，类似于：
