...
[^2]: 对命令行输入输出的支持。如果涉及到图形界面，则需要对 X 服务器进行进一步配置，取决于使用的显示类型。
...
用法很简单，但效果取决于延时长短。这里就涉及到了对程序的理解问题。如果一个过程对资源占用很大，可以考虑让其 sleep 一小会儿来减轻给机器人施加的压力。如果一个过程并非实时性强，也可以适当加入延时。
